...
*/
  ...
  vector<int> index;
  removeNaNFromPointCloud (*sensor_cloud, *sensor_cloud, index);
 ...
 */
bool DataHandler::clusterObjects (const PointCloud<PointXYZ>::Ptr & cloud_over_table)
{
  clock_t begin = clock();
  ...
  ROS_DEBUG_STREAM("Found " << cluster_indices.size () << " clusters.");
  ...
  for (size_t i = 0; i < cluster_indices.size(); ++i)
  {
    ...
    ROS_DEBUG_STREAM("Cluster " << i << " has " << cloudCluster->points.size () << " data points.");
  }
  ROS_INFO("Cluster extraction took %gms", durationInMillis(begin));
}
